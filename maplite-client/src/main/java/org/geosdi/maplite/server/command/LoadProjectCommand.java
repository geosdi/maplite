 ...
 *
 */
package org.geosdi.maplite.server.command;
...
import org.geosdi.maplite.client.command.LoadProjectRequest;
import org.geosdi.maplite.client.command.LoadProjectResponse;
 ...
 */
@Lazy(true)
@Component(value = "command.basic.styler.LoadProjectCommand")
public class LoadProjectCommand implements GPCommand<LoadProjectRequest, LoadProjectResponse> {
    ...
    @Override
    public LoadProjectResponse execute(LoadProjectRequest request,
        ...
        try {
            projectDTO = this.geoPlatformServiceClient.getProjectWithExpandedFolders(
                    request.getProjectId(), request.getAccountId());
        } catch (ResourceNotFoundFault rnf) {
            logger.error("Returning no elements: " + rnf);
        }
        GPClientProject result = this.convertToGPClientProject(projectDTO);
